	...
	public synchronized void transferencia(int cuentaOrigen, int cuentaDestino, double cantidad) throws InterruptedException{ //con synchronized nos evitamos el uso de un objeto de tipo Condition
		...
		//try {
			while(cuentas[cuentaOrigen] < cantidad) { //evalua que el saldo no es inferior a la transferencia
				//return;
				//saldoSuficiente.await(); //El hilo permanecera en espera mientras se siga cumpliendo la conidicion del while, de esta forma un hilo nunca muere solamente queda a la espera hasta que cumpla la condición
				wait(); //este método se puede usar gracias a synchroniez, misma función que await()
			}
			...
			notifyAll(); //este método se puede usar gracias a synchroniez, misma función que signalAll()
		//}finally {
			//cierreBanco.unlock();
		//}
	}
